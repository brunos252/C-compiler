...
public class SemantickiAnalizator 
{
	...
	private static CvorZn korijen_znakova=new CvorZn();
    ...
    public static void main(String[] args) 
    {
    	...
    	try(BufferedReader br = new BufferedReader(new InputStreamReader(System.in)))
    	{
    		...
    		while(line!=null && !(line.isEmpty()))
    		{
    			...
    			Cvor cv=new Cvor();
    			cv.setjedinka(pom);
    			razinaCvora.put(broj_razmaka, cv);
    			Cvor pomoc=razinaCvora.get(broj_razmaka-1);
    			pomoc.dodajDjete(cv);
        		line=br.readLine();
    		}
    ...
    private static boolean imaMain(CvorZn korijen_znakova)
    {
    	...
    	for(CvorZn.identifikator IDN:korijen_znakova.getidentifikatori())
    	{
    ...
    private static void provjeri(Cvor trenutni_cvor, CvorZn trenutni_cvor_u_tablici_znakova)
    {
    	if(trenutni_cvor.getjedinkaIDN().equals("<primarni_izraz>"))
    	{
    		...
    		{
    			//SAC1
    			//CvorZn.identifikator IDN=pretraziTabZnakova(trenutni_cvor.getdjeca().get(0),trenutni_cvor_u_tablici_znakova);
    			CvorZn.identifikator IDN = null;
    			Cvor jedinka = trenutni_cvor.getdjeca().get(0);
    			CvorZn trenutni_cvor_znak = new CvorZn();
    			trenutni_cvor_znak.clone(trenutni_cvor_u_tablici_znakova);
    			while(true) {
					for(CvorZn.identifikator idnt : trenutni_cvor_znak.getidentifikatori())
					{
						if(jedinka.getjedinkaime().equals(idnt.getime()))
						{
							IDN = new CvorZn.identifikator();
							IDN.clone(idnt);
							break;
						}
					}
					...
					{
						trenutni_cvor_znak.clone(trenutni_cvor_znak.getroditelj());
					}
					else
						{
							IDN = null;
    	...
    	else if(trenutni_cvor.getjedinkaIDN().equals("<naredba>"))
    	{
    		...
    		{
    			//novi_cvor_znak = new? i nadi sve takve novi cvor znak di nisu new
    	...
    	else if(trenutni_cvor.getjedinkaIDN().equals("<definicija_funkcije>"))
    	{
    		if(trenutni_cvor.getdjeca().get(3).getjedinkaIDN().equals("KR_VOID"))
    		{
    			...
    			CvorZn.identifikator IDN=provjeriTrenCvZnakova(trenutni_cvor.getdjeca().get(1),korijen_znakova);
    			if(IDN!=null)
    			{
    				if(IDN.getDD()==1)
    				{
    		...
    		else if(trenutni_cvor.getdjeca().get(3).getjedinkaIDN().equals("<lista_parametara>"))
    		{
    			...
    			else
    			{
    				...
    				IDN = new CvorZn.identifikator(tip_funkcije, trenutni_cvor.getdjeca().get(1).getjedinkaime(),1,false);
    				trenutni_cvor_u_tablici_znakova.dodajIdentifikator(IDN);
    				//CvorZn novi_cvor_znak=new CvorZn();
    			...
    			for(int i=0;i<tipovi.size();i++)
    			{
    				if(tipovi.get(i).size()>1)
    				{
    					...
    					IDN=new CvorZn.identifikator(tipovi.get(i),trenutni_cvor.getdjeca().get(3).getIme().get(i),1,true);
    				}
    				else
    				{
    					IDN=new CvorZn.identifikator(tipovi.get(i),trenutni_cvor.getdjeca().get(3).getIme().get(i),1,true);
    	...
    	else if(trenutni_cvor.getjedinkaIDN().equals("<izravni_deklarator>"))
    	{
    		if(trenutni_cvor.getdjeca().size()==1)
    		{
    			...
    			CvorZn.identifikator IDN=provjeriTrenCvZnakova(trenutni_cvor.getdjeca().get(0),trenutni_cvor_u_tablici_znakova);
    			if(IDN!=null)
    			{
    				greska(trenutni_cvor);
    			}
    			IDN=new CvorZn.identifikator(trenutni_cvor.getntip(),trenutni_cvor.getdjeca().get(0).getjedinkaime(),1,true);
    			trenutni_cvor_u_tablici_znakova.dodajIdentifikator(IDN);
    		...
    		else if(trenutni_cvor.getdjeca().get(2).getjedinkaIDN().equals("BROJ"))
    		{
    			...
    			IDN= new CvorZn.identifikator(p,trenutni_cvor.getdjeca().get(0).getjedinkaime(),1,true);
    			trenutni_cvor_u_tablici_znakova.dodajIdentifikator(IDN);
    			trenutni_cvor.setTip(p);
    		...
    		else if(trenutni_cvor.getdjeca().get(2).getjedinkaIDN().equals("KR_VOID"))
    		{
    			...
    			else
    			{
    				...
    				pom.add("void");
    				IDN=new CvorZn.identifikator(pom,trenutni_cvor.getdjeca().get(0).getjedinkaime(),0,false);
    				trenutni_cvor_u_tablici_znakova.dodajIdentifikator(IDN);
    			}
    			LinkedList<String> pom=new LinkedList<>();
				pom.add("fun");
	...
	private static CvorZn.identifikator provjeriTrenCvZnakova(Cvor jedinka, CvorZn trenutni_cvor_znak)
	{
		for(CvorZn.identifikator IDN: trenutni_cvor_znak.getidentifikatori())
		{
